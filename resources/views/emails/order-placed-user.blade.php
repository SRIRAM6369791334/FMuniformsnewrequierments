                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; max-width: 600px;">
                    ...
                    <tr>
                        <td style="padding: 0 20px;">
                            <h2 style="text-align: center; color: #00274D; margin: 20px 0;">Order Placed Successfully</h2>
                            <p style="text-align: center; font-size: 14px; color: #666666; margin: 10px 0 30px 0;">
                                Hello {{ $customerName }}, thank you for your order! Your order has been successfully placed and is being processed.
                            </p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding: 0 20px;">
                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse; border: 1px solid #e5e7eb; margin: 20px 0;">
                                ...
                                @if($address)
                                <tr style="background-color: #fafafa;">
                                    <td style="padding: 12px 15px; border: 1px solid #e5e7eb; font-weight: 600; width: 40%;">Name</td>
                                    <td style="padding: 12px 15px; border: 1px solid #e5e7eb;">{{ $order->order_name }}</td>
                                </tr>
                                <tr>
                                    <td style="padding: 12px 15px; border: 1px solid #e5e7eb; font-weight: 600;">Address</td>
                                    <td style="padding: 12px 15px; border: 1px solid #e5e7eb;">
                                        {{ $address->address_line_one }}<br>
                                        @if($address->address_line_two) {{ $address->address_line_two }}<br> @endif
                                        @if($address->landmark) Landmark: {{ $address->landmark }}<br> @endif
                                        {{ $address->city }}, {{ $address->state }} - {{ $address->pincode }}
                                    </td>
                                </tr>
                                <tr style="background-color: #fafafa;">
                                     <td style="padding: 12px 15px; border: 1px solid #e5e7eb; font-weight: 600;">Phone Number</td>
                                    <td style="padding: 12px 15px; border: 1px solid #e5e7eb;">{{ $address->address_phone_number }}</td>
                                </tr>
                                @else
                                <tr style="background-color: #fafafa;">
                    ...
                    <tr>
                        <td style="padding: 0 20px; text-align: center;">
                            <a href="{{ url('/account/orders/' . $order->order_id) }}" style="display: inline-block; background-color: #e11e12; color: #ffffff; padding: 12px 30px; text-decoration: none; border-radius: 4px; margin: 20px 0;">View Order Details</a>
                        </td>
                    </tr>
                    @endif
